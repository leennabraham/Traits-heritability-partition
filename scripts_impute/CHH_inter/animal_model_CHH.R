...
setwd("/projects/ag-demeaux/labraha3/EpiDom_A_lyrata/Animal_model/inter_impute/inter_CHH/input_files")

# Load the data without setting row names
df_transposed_inter <- read.csv("gBM_CHH_for_animal_model_inter_04_12.csv", header = TRUE, row.names = 1, check.names = FALSE, sep= ",")
...
variancesCalc = function(myfile, st, end, batch_info, dam_info, tray_info, samplingdate_info, outdir, individual_files = FALSE) {
  ...
  for (iter in 1:length(colnames(counts))) {
    ...
    output_gene_file = paste0(outdir, "/output_inter_CHH_", gene, ".csv")
...
variancesCalc(df_transposed_inter, st, end, batch_info, dam_info, tray_info, samplingdate_info, outdir = "/projects/ag-demeaux/labraha3/EpiDom_A_lyrata/Animal_model/inter_impute/inter_CHH/output_files/", individual_files = TRUE)