...
setwd("/projects/ag-demeaux/labraha3/EpiDom_A_lyrata/Animal_model/intra_impute/intra_CHG/input_files")

# Load the data without setting row names
df_transposed_intra <- read.csv("gBM_CHG_for_animal_model_intra_04_12.csv", header = TRUE, row.names = 1, check.names = FALSE, sep= ",")


Amat_inter = read.csv("Amat_intra_DNA_methylation.csv", header = TRUE, check.names = FALSE)
Dmat_inter = readMM("Dmat_intra_DNA_methylation.txt")
...
# Load random effects
batch_info = read.csv("batch_info_methyl_intra.csv", header = T, sep = ",")
dam_info = read.csv("dam_info_methyl_intra.csv", header = T)
tray_info = read.csv("tray_info_methyl_intra.csv", header = T)
samplingdate_info = read.csv("samplingdata_info_methyl_intra.csv", header = T)
...
variancesCalc = function(myfile, st, end, batch_info, dam_info, tray_info, samplingdate_info, outdir, individual_files = FALSE) {
  ...
  for (iter in 1:length(colnames(counts))) {
    ...
    output_gene_file = paste0(outdir, "/output_intra_CHG_", gene, ".csv")
...
variancesCalc(df_transposed_intra, st, end, batch_info, dam_info, tray_info, samplingdate_info, outdir = "/projects/ag-demeaux/labraha3/EpiDom_A_lyrata/Animal_model/intra_impute/intra_CHG/output_files", individual_files = TRUE)